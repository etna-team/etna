...
from etna.transforms.math import binary_operation
...
def test_simple_change_target(simple_ts_: TSDataset, operand):
    ...
    transformer = binary_operation.BinaryOperationTransform(left_operand="feature", right_operand="target", operator=operand, out_column="target")
...
def test_simple_add_column(simple_ts_,operand):
    ...
    feature_vals = simple_ts_.df["segment"]["feature"].values
    checker_vals = ops[operand](feature_vals, target_vals)
    transformer = binary_operation.BinaryOperationTransform(left_operand="feature", right_operand="target",
                                                            operator=operand, out_column="new_col")
    new_ts = transformer.fit_transform(ts=simple_ts_)
...
def test_inverse(simple_ts_,operand):
    target_vals = simple_ts_.df["segment"]["target"].values
    transformer = binary_operation.BinaryOperationTransform(left_operand="feature", right_operand="target",
                                                            operator=operand, out_column="target")
    new_ts = transformer.fit_transform(ts=simple_ts_)
