...
def test_save_transform(ts_non_negative, transform_original, transform_function, out_column):
    ...
    assert set(ts_copy._df.columns) == set(ts._df.columns)
    for column in ts.features:
...
def test_interface_inplace(ts_non_negative):
    ...
    original_columns = set(ts_non_negative._df.columns)
    transform.fit_transform(ts_non_negative)
    assert set(ts_non_negative._df.columns) == original_columns
...
def test_interface_not_inplace(ts_non_negative):
    ...
    original_columns = set(ts_non_negative._df.columns)
    transform.fit_transform(ts_non_negative)
    assert set(ts_non_negative._df.columns) == original_columns.union(
