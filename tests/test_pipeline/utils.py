...
def assert_pipeline_forecasts_without_self_ts(
    ...
    if ts.freq is None:
        expected_index = pd.Index(np.arange(ts.index[-1], ts.index[-1] + horizon + 1), name="timestamp")[1:]
    else:
...
def assert_pipeline_forecasts_given_ts(pipeline: AbstractPipeline, ts: TSDataset, horizon: int) -> AbstractPipeline:
    ...
    if ts.freq is None:
        expected_index = pd.Index(
            np.arange(to_forecast_ts.index[-1], to_forecast_ts.index[-1] + horizon + 1), name="timestamp"
        )[1:]
    else:
