...
def test_create_holidays_df_str_fail_as_is(simple_tsdf):
    with pytest.raises(ValueError, match="Parameter `as_is` should be used with"):
        _create_holidays_df("RU", simple_tsdf.timestamps, as_is=True)
...
def test_create_holidays_df_intersect_none(simple_tsdf):
    ...
    df = _create_holidays_df(holidays, simple_tsdf.timestamps, as_is=False)
...
def test_create_holidays_df_as_is(simple_tsdf):
    ...
    df = _create_holidays_df(holidays, simple_tsdf.timestamps, as_is=True)
    assert df.sum().sum() == 4
...
def test_create_holidays_df_as_is_int_timestamp(simple_tsdf_int_timestamp):
    ...
    df = _create_holidays_df(holidays, simple_tsdf_int_timestamp.timestamps, as_is=True)
    assert df.sum().sum() == 4
...
def test_create_holidays_df_upper_window_negative(simple_tsdf):
    ...
    with pytest.raises(ValueError):
        _create_holidays_df(holidays, simple_tsdf.timestamps, as_is=False)
