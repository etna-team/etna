...
from tests.test_prediction_intervals.common import get_naive_pipeline
from tests.test_prediction_intervals.common import get_naive_pipeline_with_transforms
from tests.test_prediction_intervals.common import run_base_pipeline_compat_check
from tests.test_prediction_intervals.utils import assert_sampling_is_valid
...
def test_backtest(example_tsds, pipeline_name, request):
    pipeline = request.getfixturevalue(pipeline_name)
    pipeline_results = pipeline.backtest(ts=example_tsds, metrics=[_DummyMetric()])["list_forecast_ts"]

    intervals_pipeline = DummyPredictionIntervals(pipeline=pipeline)
    intervals_pipeline_results = intervals_pipeline.backtest(ts=example_tsds, metrics=[_DummyMetric()])[
        "list_forecast_ts"
    ]
