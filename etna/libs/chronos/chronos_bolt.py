        ...
        **predict_kwargs,
        ...
        if set(quantile_levels).issubset(set(training_quantile_levels)):
            ...
            if min(quantile_levels) < min(training_quantile_levels) or max(
                ...
                warnings.warn(  # changed from logger.warning
                    f"Quantiles to be predicted ({quantile_levels}) are not within the range of quantiles that Chronos-Bolt was trained on ({training_quantile_levels}). Quantile predictions will be set to the minimum/maximum levels at which Chronos-Bolt was trained on. This may significantly affect the quality of the predictions."
                )
