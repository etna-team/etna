...
def mrmr(
    ...
    selected_features: List[str] = []
    relevant_features = all_features.copy()

    if drop_zero:
        not_relevant_features = list(filter(lambda feature: not relevance.loc[feature] == 0, relevant_features))
    ...
    for i in range(top_k):
        score_numerator = relevance.loc[relevant_features]
        score_denominator = pd.Series(0, index=relevant_features)
        if i > 0:
            ...
            not_selected_regressors = regressors.loc[pd.IndexSlice[:], pd.IndexSlice[:, relevant_features]]
            ...
            redundancy_table.loc[relevant_features, last_selected_feature] = (
            ...
            score_denominator = redundancy_table.loc[relevant_features, selected_features].max(axis=1)
        score = score_numerator * (1 - score_denominator)
        best_feature = score.index[score.argmax()]
        selected_features.append(best_feature)
        relevant_features.remove(best_feature)
